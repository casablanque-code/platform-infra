  ...
  config = {
    ...
    "cloud-init.user-data" = <<-CLOUDINIT
      ...
      runcmd:
        ...
        - for i in $(seq 1 5); do apt-get update -qq -o Acquire::ForceIPv4=true && apt-get install -y -qq -o Acquire::ForceIPv4=true --option Acquire::Retries=3 openssh-server && break || sleep 15; done
